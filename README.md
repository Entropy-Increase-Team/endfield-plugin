...
基于森空岛 API 的 Yunzai-Bot **终末地**插件 · 绑定 / 账号便签 / 角色列表 / 影拓丰碑 / 危机合约 / 干员面板 / Wiki / 攻略 / 抽卡 / MaaEnd
...
- 当前版本：`2.2.0`（2026-06-30）

- 支持 Cred / 网页授权 / 扫码 / 手机号绑定，支持账号便签、角色列表、影拓丰碑、危机合约、干员面板、Wiki、攻略、帝江号与地区建设等查询
...
| `:便签` / `:账号便签` | 查询账号便签 |
| `:角色` | 查询角色列表 |
| `:影拓丰碑 [名称]` | 查询影拓丰碑通关记录，可按丰碑名称筛选 |
| `:危机合约` / `:危机` / `:合约 [名称或ID]` | 查询危机合约分数、任务和挑战记录；`:危机 指标` 查看完整指标 |
| `:同步面板` / `:更新面板` | 同步展柜面板后返回已拥有干员列表（同步角色优先） |
...
- `:便签` 现已调整为账号便签视图，聚合展示账号信息与统计数据
- `:角色` 为独立角色列表页，角色卡支持潜数、星级图标与武器信息展示
- `:同步面板` / `:更新面板` 用于主动同步展柜面板，返回最新已拥有干员列表
...
| 命令 | 说明 |
|------|------|
| `:抽卡分析 [用户平台ID]` | 查看抽卡分析（无数据时自动同步，管理员可指定平台ID） |
| `:抽卡记录` | 查看抽卡记录（无数据时自动同步） |
| `:同步抽卡记录 [用户平台ID]` / `:更新抽卡记录 [用户平台ID]` | 手动同步抽卡记录（管理员可指定平台ID） |
| `:全服抽卡统计` | 查看全服抽卡统计 |
